...
/** Full navigation + print PDF (demo `/api/pdf*` routes using hosted HTML URLs). */
export async function puppeteerPdfFromUrl(url, opts) {
  const { salesFlowCompaction } = opts;
  const browser = await puppeteer.launch({
    headless: true,
    args: ['--no-sandbox', '--disable-setuid-sandbox'],
  });
  try {
    ...
    await page.emulateMediaType('print');
    await applyPdfLetterTableLayout(page);
    if (salesFlowCompaction) await applyPdfSalesFlowCompaction(page);
    await applyPdfOrphanCompaction(page);
    const pdfBuffer = await page.pdf({
      width: '210mm',
      height: '297mm',
