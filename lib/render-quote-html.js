...
/** @type {{ scale: number, baseUrl: string, pdfFontScale: number, lineTotalColIndex?: number, spacerColSpan?: number }} */
  ...
  const renderRow = (row, ri, totalRows) => {
    ...
    for (let ci = 0; ci < norm.length; ci++) {
      ...
      if (lightLines) {
        ...
        const border = salesPdf
          ? 'border:1px solid #ccc'
          : !isLastRow
            ? 'border-bottom:1px solid #ccc'
            : '';
        const va = salesPdf ? 'top' : 'middle';
        parts.push(
...
function renderBlock(block) {
  ...
  if (block.table) {
    ...
    if (_opts.salesPdf === true && block._salesMainItemsTable) {
      return `<div class="qt-sales-main-items-wrap">${html}</div>`;
    }
    return html;
  }
...
/** Top band (header + contact strip) — ~12–15% of page in artwork */
const SAFE_HEADER_BAND_PCT = 0.15;
/** Bottom band (partner logos strip) — ~8–11% of page */
const SAFE_FOOTER_BAND_PCT = 0.11;
