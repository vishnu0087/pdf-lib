/**
 * Salary slip PDF: `page2.afterTable` (pdfmake-style), single letter sheet, same shell as sales/invoice.
 * JSON: `puppeteer-salary-pdf` (`fixtures/salary/sa-data1.json`). Shell: `pdf/salary-template.tsx`.
 */
 ...
 * @param {object} raw
 */
export function normalizeSalaryData(raw) {
  if (!raw || typeof raw !== 'object') return raw;
  if (raw.format === 'puppeteer-salary-pdf') {
    return raw;
  }
  if (Array.isArray(raw.content)) {
    const m = raw.pageMargins || [22, 90, 22, 36];
    const inset = {
      left: Number(m[0]) || 22,
      top: Math.max(Number(m[1]) || 90, 100),
      right: Number(m[2]) || 22,
      bottom: Math.max(Number(m[3]) || 36, 36),
    };
    return {
      format: 'puppeteer-salary-pdf',
      formatVersion: 1,
      document: {
        defaultStyle: raw.defaultStyle || { font: 'Roboto', fontSize: 8, color: '#222' },
        styles: raw.styles || {},
        pdfFontScale: 1,
...
function normalizeSalaryShell(data) {
...
function normalizeImagePaths(node) {
  ...
  if (out.image && typeof out.image === 'string') {
    ...
    } else if (s.includes('not_approved.png') && !s.includes('stamp')) {
...
function splitSalaryLineItemsAndGrandTotals(blocks) {
 ...
 */
export function renderSalaryDocumentParts(data, baseUrl) {
  const normalized = normalizeSalaryData(data);
  const shellData = normalizeSalaryShell(normalized);
  ...
  const split = splitSalaryLineItemsAndGrandTotals(afterBlocks);
  let bodyInner = '';
  ...
  const bodyWrapped = bodyInner
    ? `<div class="p2-table-wrap"><div class="p2-after-table p2-after-table--sales p2-after-table--salary">${bodyInner}</div></div>`
    : '';
