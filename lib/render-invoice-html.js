/**
 * Invoice PDF: own pipeline file (body layout matches sales: page2.afterTable, single letter sheet).
 * JSON format: `puppeteer-invoice-pdf` (see `fixtures/invoice/invoice-data1.json`). Shell: `pdf/invoice-template.tsx`.
 */
...
/**
 * Flat pdfmake-style JSON → `puppeteer-invoice-pdf` shape, or pass through when already in that form.
 */
export function normalizeInvoiceData(raw) {
  if (!raw || typeof raw !== 'object') return raw;
  if (raw.format === 'puppeteer-invoice-pdf') {
    return raw;
  }
...
function normalizeInvoiceShell(data) {
...
function splitInvoiceLineItemsAndGrandTotals(blocks) {
...
/**
 * @param {object} data Parsed invoice JSON (puppeteer-invoice-pdf or flat)
 ...
 */
export function renderInvoiceDocumentParts(data, baseUrl) {
  const normalized = normalizeInvoiceData(data);
  const shellData = normalizeInvoiceShell(normalized);
  ...
  const split = splitInvoiceLineItemsAndGrandTotals(afterBlocks);
  let bodyInner = '';
